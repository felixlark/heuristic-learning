...
| `cross-example-report-reading` | `cross-example-analysis` | 七个 runnable examples | `npm run examples:reports:check`、`npm run benchmark:summary:check`、`npm run metrics:check` | 为每个报告补人工可读 edit hint |
| `constraint-audit-related-work` | `public-essay` | `constraint-audit` | `npm run examples:constraint-audit:feedback`、`npm run claims:registry:check` | 用外部证据与独立核验测量误报、漏报和拒答 |
...
| `cross-example-report-reading` | 比较七个 latest.json 是否足以定位 failure mode 和 update target。 |
| `constraint-audit-related-work` | 把自检与核验相关工作转成闭世界约束冲突和证据升级的可反驳研究夹具。 |

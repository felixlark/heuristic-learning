---
title: 研究日志
description: 把阅读、代码运行、研究命题和下一步实验连成可检查记录
---

# 研究日志

研究日志回答一个教学仓库里的实际问题：学生或研究者读完一篇文章、一段 artifact 代码、一条 X 线索或一个内部应用方向之后，如何留下下一轮可以继续推进的证据。它把 [文献阅读指南](/zh-cn/appendix/reading-guide)、[案例矩阵](/zh-cn/appendix/case-registry)、[评估指标](/zh-cn/appendix/evaluation-metrics) 和 [论文蓝图](/zh-cn/appendix/paper-blueprint) 连接起来。

机器可读日志见 [`/research-logbook.json`](/research-logbook.json)，字段约束见 [`/research-logbook.schema.json`](/research-logbook.schema.json)。`npm run research:logbook:check` 会检查每条记录的来源、绑定示例、研究命题、指标、命令、交付物和边界。

## 记录格式

| 字段 | 含义 |
| --- | --- |
| `source_type` | 来源类别：公开文章、公开代码 artifact、X/FieldTheory 线索、内部线索或跨示例分析 |
| `reading_goal` | 本次阅读要抽取的可教学问题 |
| `bound_examples` | 读完后应该能落到哪些 runnable example |
| `bound_claims` | 影响哪些研究命题，而不是直接写成结论 |
...
| `x-signal-case-reading` | `x-or-fieldtheory-signal` | `breakout-replay`、`ant-gait-replay` | `npm run x:sources:check`、`npm run source:case:check` | 用 `ft` 或受支持的 X API 刷新一手帖缓存 |
| `robot-soccer-signal-reading` | `internal-signal` | `robot-soccer` | `npm run examples:robot-soccer:feedback`、`npm run examples:test` | 扩展安全通道场景 |
...
| `breakout-artifact-reading` | 把 Breakout artifact 的几何预测思想压缩成学生能读懂和能测试的 replay。 |
| `x-signal-case-reading` | 把 Jiayi Weng 相关 X 线索拆成来源状态、可课程化问题和禁止声明。 |
| `robot-soccer-signal-reading` | 把内部机器人足球方向转成脱敏、可运行、可测试的 blocked-lane 教学案例。 |
| `cross-example-report-reading` | 比较六个 latest.json 是否足以让下一轮编码智能体定位 failure mode 和 update target。 |

## 使用方式

1. 先从 [文献阅读指南](/zh-cn/appendix/reading-guide) 或 [来源到案例 Playbook](/zh-cn/appendix/source-to-case-playbook) 选择来源。
2. 对公开 artifact、X/FieldTheory 线索或脱敏内部线索，先用 `templates/reproduction-note.md` 写清来源状态、复现范围、缺失保真度、反驳路径和下一步实验。
3. 写清楚本次阅读只想抽取一个问题，不把整篇文章或整段代码一次性搬进仓库。
4. 绑定一个或多个 runnable example，并列出最短命令。
5. 绑定研究命题和评估指标，说明它支持、削弱或只是提出问题。
6. 写出 `boundary`，尤其是 `referenced-not-cached`、轻量 replay、内部资料脱敏和未跑真实环境这些边界。
...
- `templates/claim-review.md`：把阅读记录升级为研究命题审查时使用。

...
| Constraint Audit research gap | `examples/constraint-audit` 把事实性问题压缩成给定目录的冲突检查 | 开放世界检索、时变主张、独立生成与核验 | 绑定带时间戳的原子事实和外部证据，再评估错误相关性 | `npm run examples:constraint-audit:feedback`、`npm run claims:registry:check` |

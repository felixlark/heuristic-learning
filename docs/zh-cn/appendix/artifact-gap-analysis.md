...
本页把“当前示例已经跑通”和“真实 artifact 已经复现”分开。它回答一个研究仓库必须面对的问题：每个轻量 replay 到 Jiayi 公开 artifact、内部线索或教学环境之间还差哪些保真度，下一步应该先补哪个实验。
...
4. 如果来源来自飞书或其他内部系统，只能发布脱敏后的失败模式、字段和不变量。

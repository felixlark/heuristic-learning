...
机器可读矩阵见 [`/evaluation-metrics.json`](/evaluation-metrics.json)，字段约束见 [`/evaluation-metrics.schema.json`](/evaluation-metrics.schema.json)。`npm run metrics:check` 会检查指标、示例、研究问题、证据路径、验证命令和 manifest 是否一致。
...
2. 新增研究问题时，至少要绑定一个评估指标，说明它如何被反驳。

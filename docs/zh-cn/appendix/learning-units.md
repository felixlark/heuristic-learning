...
| U4 研究命题 | 当前证据支持哪些命题，边界在哪里？ | [研究框架](/zh-cn/theory/research-framework)、[研究命题](/zh-cn/theory/research-propositions)、[论文蓝图](/zh-cn/appendix/paper-blueprint) | `npm run claims:registry:check`、`npm run paper:blueprint:check` | 命题-证据-反驳路径说明 |
...
学生可以按 U0 到 U5 顺序完成；研究者可以从 U2 或 U4 切入；教师可以把每个单元拆成一次课堂或一次组会任务。编码智能体在维护仓库时，应优先查看 `/learning-units.json`，确认新增页面或示例是否改变了已有单元的阅读、命令或交付物。

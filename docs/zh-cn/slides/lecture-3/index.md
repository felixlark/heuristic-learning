...
- 把失败区分为局部贪心、动作前提缺失、资源时机错误、物理预测不足、控制稳定性退化和系统容量约束缺失。
- 判断一次 patch 是否留下了 failure probe、feedback report 和 regression gate。
- 预测新 heuristic 可能破坏的旧行为，并把它写进反遗忘检查。
...
一次 patch 要成为 HL 学习结果，至少要留下三类证据：
...
学习每个案例时都问同一个问题：新 heuristic 会不会破坏旧行为？

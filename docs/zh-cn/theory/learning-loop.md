...
Signal 可以来自 X 线程、公开仓库 artifact、脱敏来源、日志、视频回放或一次失败测试。进入课程前，必须先回答：
...
例子：[X 来源案例](/zh-cn/cases/x-signal/) 把 Jiayi 原帖和中文转述拆开记录；[来源登记](/zh-cn/appendix/source-registry) 再标注它们在课程里的状态。

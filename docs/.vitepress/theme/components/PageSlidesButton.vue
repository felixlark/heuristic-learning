...
const createSlideVisualCard = (role) => {
  ...
  badge.textContent = role === 'cover' ? 'HL Loop' : 'Learning Step'

  const title = document.createElement('strong')
  title.textContent = role === 'cover' ? '从反馈到更新' : '一个概念一轮验证'
  ...
  footer.textContent = '把失败信号压缩成可运行、可测试、可复盘的 heuristic 更新'
